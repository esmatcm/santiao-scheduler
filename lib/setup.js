...
async function launchAndVerify() {
  ...
  // Check up to 5 times (15 seconds total)
  for (let i = 0; i < 5; i++) {
    try {
  ...
  return {
    ok: false,
    error: 'App launched but chat interface not detected. The app may need login or is still loading. Check the phone screen.',
  };
}
...
module.exports = { getState, isComplete, checkAdb, checkDevice, installSantiao, launchAndVerify, installIme, verify, reset };
